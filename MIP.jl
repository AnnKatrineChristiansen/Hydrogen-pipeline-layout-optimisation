...
function run_mip(instance, zone, number_of_manifolds, pressure, strings, timelimit, folder_name, crossings, it = 0)
    ...
    # At most 10 enters a manifold
    @constraint(model,[h = 1:S], sum(i == h ? 0 : y[i,h] for i=1:N) <= 10)
    ...
    new_crossings = evaluate_termination(timelimit, model, x, f, y, N, T, instance, zone, data, manifold_array, folder_name)
    println("Crossings", new_crossings)
    
    if !isempty(new_crossings) && it < 6
...
function evaluate_termination(timelimit, model, x, f, y, N, T, instance, zone, data, manifold_array, folder_name)
    ...
    if termination_status(model) == MOI.TIME_LIMIT && primal_status(model) == MOI.FEASIBLE_POINT
        ...
        if isempty(crossings)
            row = [instance, zone, termination, best_bound, timelimit, best_objective, sol_sizing.cost, cal_mip.feasible, cal_sizing.feasible]
        else
            return crossings 
        end 
    ...
    elseif termination_status(model) == MOI.OPTIMAL
        ...
        row = [instance, zone, termination, best_bound, timelimit, best_objective, NaN, NaN, NaN]
